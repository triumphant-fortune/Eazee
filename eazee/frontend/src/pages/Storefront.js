...
export default function Storefront() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="w-8 h-8 border-4 border-orange-400 border-t-transparent rounded-full animate-spin" /></div>;

  if (notFound) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="text-center"><div className="text-6xl mb-4">🔍</div><h1 className="text-xl font-semibold text-gray-800">Store not found</h1><p className="text-gray-400 mt-2 text-sm">This store link doesn't exist or has been removed.</p></div>
    </div>
  );

  const palette = PALETTES[vendor?.palette] || PALETTES.warm;

  // Inactive store display
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ backgroundColor: palette.bg }}>
      <div className="text-center max-w-sm">
        ...
        {vendor.whatsapp_number && (
          <a
            href={`https://wa.me/${vendor.whatsapp_number.replace(/\D/g, '')}?text=Hi! I visited your Eazee store and wanted to reach out.`}
            target="_blank"
            rel="noreferrer"
            className="mt-8 inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-white font-semibold text-sm transition"
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="py-8 px-4" style={{ backgroundColor: palette.bg }}>
        <div className="max-w-lg mx-auto text-center">
          {vendor.logo_url ? (
            ...
            <div className="w-20 h-20 rounded-2xl mx-auto mb-3 flex items-center justify-center text-white text-3xl font-bold shadow-sm" style={{ backgroundColor: palette.primary }}>
              {vendor.business_name?.[0]}
            </div>
          )}
          <h1 className="text-2xl font-bold text-gray-900">{vendor.business_name}</h1>
          {vendor.bio && <p className="mt-2 text-sm text-gray-500 max-w-xs mx-auto leading-relaxed">{vendor.bio}</p>}
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        {products.length === 0 ? (
          <div className="text-center py-16 text-gray-400"><div className="text-5xl mb-3">📦</div><p className="font-medium">No products yet</p><p className="text-sm mt-1">Check back soon!</p></div>
        ) : (
          <>
            <p className="text-xs text-gray-400 mb-4 font-medium">{products.length} product{products.length !== 1 ? 's' : ''} available</p>
            <div className="grid grid-cols-2 gap-3">
              {products.map(product => {
                const stockTag = STOCK_TAGS[product.stock_tag];
                const isOutOfStock = product.stock_tag === 'out_of_stock';
                return (
                  <button key={product.id} onClick={() => !isOutOfStock && setSelectedProduct(product)}
                    className={`bg-white rounded-2xl overflow-hidden border border-gray-100 text-left transition-shadow ${isOutOfStock ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-md active:scale-95 transition-transform'}`}>
                    <div className="aspect-square bg-gray-100 overflow-hidden relative">
                      {product.media_url ? (
                        product.media_type === 'video' ? <video src={product.media_url} className="w-full h-full object-cover" muted /> : <img src={product.media_url} alt={product.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: palette.light }}><span className="text-3xl">🛍</span></div>
                      )}
                      {isOutOfStock && <div className="absolute inset-0 bg-white bg-opacity-60 flex items-center justify-center"><span className="text-xs font-bold text-red-500 bg-white px-2 py-1 rounded-full">Out of stock</span></div>}
                    </div>
                    <div className="p-3">
                      ...
                      {stockTag && !isOutOfStock && (
                        <span className="inline-block mt-1 text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: stockTag.bg, color: stockTag.color }}>{stockTag.label}</span>
                      )}
                    </div>
                  </button>
                );
      ...
      <div className="text-center py-8 text-xs text-gray-300">Powered by <span className="font-semibold text-gray-400">Eazee</span></div>

      {/* Product modal */}
      {selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end justify-center" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white w-full max-w-lg rounded-t-3xl p-5 pb-8" onClick={e => e.stopPropagation()}>
            {selectedProduct.media_url && (
              <div className="w-full h-56 rounded-2xl overflow-hidden mb-4 bg-gray-100">
                {selectedProduct.media_type === 'video' ? <video src={selectedProduct.media_url} className="w-full h-full object-cover" controls /> : <img src={selectedProduct.media_url} alt={selectedProduct.name} className="w-full h-full object-cover" />}
              </div>
            )}
            ...
            <a href={buildWhatsAppLink(selectedProduct)} target="_blank" rel="noreferrer"
              className="mt-5 flex items-center justify-center gap-2 w-full py-4 rounded-2xl text-white font-semibold text-base transition active:scale-95"
              style={{ backgroundColor: '#25D366' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              Message Vendor
            </a>
            <button onClick={() => setSelectedProduct(null)} className="mt-3 w-full py-3 rounded-2xl text-gray-500 text-sm font-medium border border-gray-100 hover:bg-gray-50 transition">Close</button>
          </div>
        </div>
      )}
    </div>
